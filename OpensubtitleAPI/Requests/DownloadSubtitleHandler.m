...
-(NSData *)gunzippedData:(NSData *) gzipdata
{
    if ([gzipdata length])
    {
        ...
        NSMutableData *data = [NSMutableData dataWithLength:(NSUInteger)([gzipdata length] * 1.5)];
    ...
    if ([response isFault]) {
        ...
        if ([responseDic isKindOfClass:[NSDictionary class]]) {
            for (NSDictionary * file in responseDic[@"data"]) {
                ...
                NSData * binaryData = [self base64DataFromString:data];
                NSData * unCompressedData = [self gunzippedData:binaryData];
